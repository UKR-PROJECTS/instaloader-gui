...
src_parent_path = os.path.join(project_root, "src", "transcriptionDisabled")
if not os.path.isdir(os.path.join(src_parent_path, "src")):
    src_parent_path = os.path.join(project_root, "src", "transcriptionEnabled")
sys.path.insert(0, src_parent_path)
...
class TestReelDownloader(unittest.TestCase):
    ...
    def test_extract_shortcode_reel(self):
        ...
        shortcode = self.downloader._extract_shortcode(url)
    ...
    def test_download_process_calls(
        self, mock_open_file, mock_mkdir, mock_moviepy, mock_requests, mock_instaloader
    ):
        """Test that the main download process calls the correct helper methods."""
        ...
        self.downloader._download_video = MagicMock()
        self.downloader._download_thumbnail = MagicMock()
        self.downloader._extract_audio = MagicMock()
        self.downloader._save_caption = MagicMock()
        ...
        self.downloader._download_video.assert_called_once()
        self.downloader._download_thumbnail.assert_called_once()
        self.downloader._extract_audio.assert_called_once()
        self.downloader._save_caption.assert_called_once()
