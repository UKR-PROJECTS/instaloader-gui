...
from src.utils.lazy_imports import lazy_import_instaloader, lazy_import_requests, lazy_import_moviepy
...
def download_reel(
    ...
    try:
        ...
        _download_video(post, reel_folder, reel_number, result, download_options, progress_callback)
        _download_thumbnail(post, reel_folder, reel_number, result, download_options, progress_callback)
        _extract_audio(reel_folder, reel_number, result, download_options, progress_callback)
        _save_caption(post, reel_folder, reel_number, result, download_options, progress_callback)
...
def _download_video(post, reel_folder: Path, reel_number: int, result: Dict, download_options: Dict, progress_callback: Any):
    """Download video file if enabled."""
    need_video_for_audio = download_options.get("audio", False) or download_options.get("transcribe", False)
...
def _download_thumbnail(post, reel_folder: Path, reel_number: int, result: Dict, download_options: Dict, progress_callback: Any):
    ...
    if hasattr(post, "thumbnail_url"):
        ...
        raise AttributeError(f"Cannot find thumbnail URL on Post object; available attributes: {dir(post)}")
    try:
        requests_module = lazy_import_requests()
...
def _save_caption(post, reel_folder: Path, reel_number: int, result: Dict, download_options: Dict, progress_callback: Any):
